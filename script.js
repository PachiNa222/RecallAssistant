...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateTemplateControls(); // ボタン状態の初期化
    renderMemories();
    renderThoughts();
    ...
    function updateTemplateControls() {
        ...
        if (deleteTemplateBtn) {
            deleteTemplateBtn.style.display = isCustom ? 'block' : 'none';
        }
        if (exportBtn) {
            exportBtn.disabled = !isCustom;
        }
    }

    // ★プルダウンの内容更新
    function updateTemplateDropdown() {
        if (!presetGroup || !customGroup) return;
    ...
    window.toggleModalInputs = function(type) {
        if (type === 'category') {
    ...
    window.toggleCollapse = function(index) {
    ...
    window.deleteKnowledge = function(catIndex, kIndex) {
    ...
    window.editKnowledge = function(catIndex, kIndex) {
    ...
    window.editThoughtName = function(index) {
    ...
    window.handleDrop = function(e, thoughtIndex) {
        e.preventDefault();
    ...
    // --- ★メニュー開閉ロジック（修正版） ---
    // ここが動かなかった原因と思われるため、確実に要素があるか確認してイベントを設定します
    if (settingsToggle && settingsContent) {
        settingsToggle.addEventListener('click', () => {
            // style.display をチェックして切り替え
